...
struct ExerciseSelectionView: View {
    ...
    @Query(filter: #Predicate<Exercise> { $0.isActive == true }, sort: [SortDescriptor(\Exercise.nameTR)]) 
    ...
    var filteredExercises: [Exercise] {
        // Since exercises already filtered by isActive and sorted, start with optimized base
        var result = exercises

        // Segment filter (most selective first for performance)
        ...
        case .recent:
            ...
            result = result.filter { recentIdSet.contains($0.id) }
        case .all:
            break
        }

        // Part type filter with cached allowed categories
        if let partType = selectedPartType {
            let allowedCategories = Set(partType.suggestedExerciseCategories.map { $0.rawValue })
            result = result.filter { allowedCategories.contains($0.category) }
        }

        // Search filter (most expensive, so do it last)
        if !debouncedSearchText.isEmpty {
            let searchLower = debouncedSearchText.lowercased()
            result = result.filter { exercise in
                exercise.nameTR.lowercased().contains(searchLower) ||
                exercise.nameEN.lowercased().contains(searchLower)
            }
        }
    ...
    private var recentExercises: [Exercise] {
        // Use Set for O(1) lookup and maintain order by recent usage
        let recentIdSet = Set(recentIds)
        let recentExercises = exercises.filter { recentIdSet.contains($0.id) }
        
        // Sort by recent order (maintain user's usage order)
        return recentExercises.sorted { first, second in
            guard let firstIndex = recentIds.firstIndex(of: first.id),
                  let secondIndex = recentIds.firstIndex(of: second.id) else {
                return first.nameTR < second.nameTR
            }
        ...
        VStack(spacing: 0) {
                ...
                ScrollView {
                    LazyVStack(spacing: 8) {
                        ...
                        // All filtered list or empty state
                        if mainExercisesExcludingRecent.isEmpty {
                            EmptyExerciseState(searchText: debouncedSearchText)
                                .padding(.top, 40)
                        } else {
